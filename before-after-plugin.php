...
function beforeafter_handle_bulk_import() {
    if ( ! isset( $_POST['submit'] ) || ! isset( $_POST['beforeafter_bulk_import_nonce_field'] ) ) { return; }
    if ( ! wp_verify_nonce( $_POST['beforeafter_bulk_import_nonce_field'], 'beforeafter_bulk_import_nonce' ) ) { wp_die('Nonce verification failed!'); }
        ...
        if ( ( $handle = fopen( $csv_path, "r" ) ) !== FALSE ) {
            ...
            while ( ( $data = fgetcsv( $handle, 1000, "," ) ) !== FALSE ) {
                ...
                if ( $post_id ) {
                    ...
                    if ($before_image_id) update_post_meta($post_id, '_beforeafter_before_image_id', $before_image_id);
                    ...
                    if ($geojson_file_id) update_post_meta($post_id, '_beforeafter_geojson_file_id', $geojson_file_id);
            ...
            if ( ! empty( $import_successes ) ) { add_action( 'admin_notices', function() use ( $import_successes ) { echo '<div class="notice notice-success is-dismissible"><p>' . implode( '<br>', $import_successes ) . '</p></div>'; }); }
            if ( ! empty( $import_warnings ) ) { add_action( 'admin_notices', function() use ( $import_warnings ) { echo '<div class="notice notice-warning is-dismissible"><p>' . implode( '<br>', $import_warnings ) . '</p></div>'; }); }
            
        } else {
            add_action( 'admin_notices', function() { echo '<div class="notice notice-error is-dismissible"><p>' . __( 'Could not open the CSV file.', 'beforeafter' ) . '</p></div>'; });
        }
    } else {
         add_action( 'admin_notices', function() { echo '<div class="notice notice-error is-dismissible"><p>' . __( 'Please upload a CSV file.', 'beforeafter' ) . '</p></div>'; });
    }
}
...
function beforeafter_render_bulk_import_page() {
    ...
    <div class="wrap">
        ...
        <form method="post" enctype="multipart/form-data" action="edit.php?post_type=beforeafter&page=beforeafter-bulk-import">
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if ($post_id) {
                    ...
                    $fields_to_import = array(
                        'sitecode', 'sitename', 'countrycode', 'perclogged', 'site_ha', 'most_disturbed_year_ha', 'most_disturbed_year'
                    );
